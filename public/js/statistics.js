...
var Statistics = (function () {
  ...
  function cleanup() {
    ...
    if (_state.requestController) {
      _state.requestController.abort();
      _state.requestController = null;
    }
  }
  ...
  function _extractRecentLoadError(data) {
    if (!data) return t('stats.load_failed');
    ...
    return t('stats.load_failed');
  }

  function _requestRecent(qs, signal) {
    return apiRaw('GET', '/stats/recent' + qs, { signal: signal }).then(function (res) {
      return res.json().then(function (payload) {
        if (!res.ok) throw new Error(_extractRecentLoadError(payload));
        return payload;
      });
    });
  }
  ...
  function _loadData() {
    ...
    _state.requestController = controller;

    _requestRecent(qs, controller.signal).then(function (result) {
      ...
      if (_state.requestController === controller) {
        _state.requestController = null;
      }
    });
  }
  ...
  function _renderSummaryBar(d) {
    ...
    for (var i = 0; i < callers.length; i++) {
      var c = callers[i] || {};
      var callerDisplay = _buildCallerDisplay(c);
      var shortIdentity = callerDisplay.text.length > 24
        ? callerDisplay.text.substring(0, 22) + '..'
        : callerDisplay.text;
      html += '<span class="stats-badge stats-badge-info" title="' + escapeHtml(callerDisplay.title) + '">'
        + escapeHtml(t('stats.caller')) + ': ';
      if (callerDisplay.avatar_url) {
        html += '<span class="abuse-avatar-wrap"><img class="abuse-avatar" src="' + escapeHtml(callerDisplay.avatar_url) + '" alt=""></span> ';
      }
      html += escapeHtml(shortIdentity) + ' ' + _fmtNum(c.requests || 0)
        + '</span>';
    }
    el.innerHTML = html;
  }
  ...
  function _renderTable() {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      html += '<tr class="stats-row' + (isExpanded ? ' is-expanded' : '') + '">'
        ...
        + '<td class="td-mono td-email" title="' + escapeHtml(_fmtCallerTitle(r)) + '">' + _fmtCaller(r) + '</td>'
  ...
  function _renderExpandedRow(r) {
    ...
    return '<tr class="stats-expand-row"><td colspan="11"><div class="stats-detail-grid">'
      ...
      + _detailKV(t('stats.caller'), _fmtCallerTitle(r))
  ...
  function _renderPagination() {
    ...
    html += '<div class="stats-per-page">'
      + t('stats.per_page') + ': ';
    var perPageOptions = [25, 50, 100, 200];
  ...
  function _buildCallerDisplay(row) {
    var r = row || {};
    var identity = String(r.caller_identity || r.identity || '').trim();
    var seqId = String(r.seq_id || '').trim();
    var discordUserId = String(r.discord_user_id || '').trim();
    var avatarUrl = String(r.avatar_url || '').trim();
    var username = String(r.display_name || r.username || '').trim();
    if (!username) {
      var key = 'stats.caller_identity.' + identity;
      username = t(key);
      if (!username || username === key) username = identity || '-';
    }
    if (username && ((seqId && username === seqId) || (discordUserId && username === discordUserId))) {
      username = '';
    }
    var text = username || identity || '-';
    if (seqId) {
      text = seqId + (username ? (' (' + username + ')') : '');
    }
    if (!text) text = '-';
    var title = text;
    if (discordUserId && title.indexOf(discordUserId) < 0) title += ' [' + discordUserId + ']';
    return {
      text: text,
      title: title,
  ...
  function _fmtCallerTitle(row) {
    return _buildCallerDisplay(row).title;
  }

  function _fmtCaller(row) {
    var display = _buildCallerDisplay(row);
    var short = display.text.length > 32 ? display.text.substring(0, 30) + '..' : display.text;
    var html = '';
    if (display.avatar_url) {
      html += '<span class="abuse-avatar-wrap"><img class="abuse-avatar" src="' + escapeHtml(display.avatar_url) + '" alt=""></span>';
    }
