...
var Dashboard = (function () {
  ...
  function _renderCallerTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var identityShort = displayName.length > 28 ? displayName.substring(0, 26) + '..' : displayName;
      html += '<tr>'
        + '<td class="td-mono td-email" title="' + escapeHtml(displayName) + '">' + escapeHtml(identityShort) + '</td>'
        + '<td class="td-mono">' + _fmtNum(req) + '</td>'
