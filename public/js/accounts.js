...
var Accounts = (function () {
  ...
  function _buildListUrl() {
    var query = [
      'page=' + encodeURIComponent(String(_page)),
      'limit=' + encodeURIComponent(String(_limit)),
    ];
    if (_statusFilter) query.push('status=' + encodeURIComponent(_statusFilter));
    if (_searchFilter) query.push('search=' + encodeURIComponent(_searchFilter));
    return '/accounts?' + query.join('&');
  }

  function _bindPaginationEvents() {
    if (_paginationBound) return;
    var box = document.getElementById('accountsPagination');
    if (!box) return;
    box.addEventListener('click', function (e) {
      var btn = e.target.closest('button[data-page]');
      if (!btn || btn.disabled) return;
      var targetPage = _toInt(btn.getAttribute('data-page'), _page);
      if (targetPage < 1 || targetPage === _page) return;
      load({ page: targetPage });
    });
  ...
  function _renderPagination() {
    ...
    var derivedPages = Math.max(1, Math.ceil((_total || 0) / _limit));
    var totalPages = Math.max(1, _toInt(_pages, derivedPages));
    if (totalPages < derivedPages) totalPages = derivedPages;
    if (_page > totalPages) _page = totalPages;
    if (_page < 1) _page = 1;

    var prevDisabled = _page <= 1 ? ' disabled' : '';
    var nextDisabled = _page >= totalPages ? ' disabled' : '';
    var prevPage = Math.max(1, _page - 1);
    var nextPage = Math.min(totalPages, _page + 1);

    box.innerHTML =
      '<button class="btn btn-sm btn-secondary" data-page="' + prevPage + '"' + prevDisabled + '>&lt; 上一页</button>' +
      ' <span>|</span> ' +
      '<button class="btn btn-sm btn-secondary" data-page="' + _page + '">第' + _page + '页/共' + totalPages + '页</button>' +
      ' <span>|</span> ' +
      '<button class="btn btn-sm btn-secondary" data-page="' + nextPage + '"' + nextDisabled + '>下一页 &gt;</button>';
  }
  ...
  function load(options) {
    ...
    if (options && options.page !== undefined && options.page !== null) {
      _page = _toInt(options.page, _page);
      if (_page < 1) _page = 1;
    }
    api('GET', _buildListUrl())
      .then(function (data) {
        ...
        _total = _toInt(data.total, _accounts.length);
        if (_total < 0) _total = 0;
        _page = _toInt(data.page, _page);
        if (_page < 1) _page = 1;
        _limit = _toInt(data.limit, _limit);
        if (_limit < 1) _limit = 50;
        _pages = _toInt(data.pages, Math.ceil((_total || 0) / _limit));
        if (_pages < 1) _pages = 1;
        if (_page > _pages) _page = _pages;
        _stats = data && typeof data.stats === 'object' && data.stats ? data.stats : null;
  ...
  function setFilter(filter) {
    ...
    _statusFilter = _currentFilter === 'all' ? '' : _currentFilter;
  ...
  function render() {
    ...
    if (_stats && typeof _stats === 'object') {
      counts.all = _toInt(_stats.total, _total);
      counts.active = _toInt(_stats.active, 0);
      counts.cooldown = _toInt(_stats.cooldown, 0);
      counts.banned = _toInt(_stats.banned, 0);
      counts.expired = _toInt(_stats.expired, 0);
      counts.wasted = _toInt(_stats.wasted, 0);
    } else {
      for (var ci = 0; ci < _accounts.length; ci++) {
        var s = _accounts[ci].status;
        // 历史状态映射：relogin_needed -> wasted，usage_limited -> cooldown
        var mapped = s === 'relogin_needed' ? 'wasted' : s === 'usage_limited' ? 'cooldown' : s;
    ...
    if (_currentFilter !== 'all') {
      filtered = filtered.filter(function (a) {
        var s = a.status;
        if (_currentFilter === 'expired') return s === 'expired';
        if (_currentFilter === 'cooldown') return s === 'cooldown' || s === 'usage_limited';
        if (_currentFilter === 'wasted') return s === 'wasted' || s === 'relogin_needed';
  ...
  function testBatch() {
    var count = (_stats && _stats.active) ? _stats.active : _accounts.filter(function(a) { return a.status === 'active'; }).length;
    if (!count) { toast('没有活跃账号可测试', 'info'); return; }
    showConfirm('确认对全部 ' + count + ' 个活跃账号发送测试请求？(50并发)', '批量测试').then(function(ok) {
      if (!ok) return;
      var btn = document.getElementById('btnTestBatch');
      if (btn) { btn.disabled = true; btn.querySelector('span').textContent = '测试中...'; }
      api('POST', '/accounts/test-batch', { filter: 'active', concurrency: 50 })
        .then(function(d) {
