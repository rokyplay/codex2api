...
function buildUsagePayload(stats, identity) {
  var safeIdentity = String(identity || '');
  var summary = buildUsageSummary(stats, safeIdentity);
  return {
    identity: safeIdentity,
    today: toObject(summary.today),
    total: toObject(summary.total),
  };
}
...
export function createDiscordAuthRoutes(config) {
  ...
  async function getSessionFromCookie(req) {
    ...
    if (!discordUserId) {
      discordUserId = _extractDiscordUserId(sessionObj.user || {}, userStore);
    }
    return {
  ...
  async function flushUserStoreIfSupported() {
    if (typeof userStore.saveUsers !== 'function') {
      throw new Error('userStore.saveUsers 不可用');
    }
    await maybeAwait(userStore.saveUsers());
  }
  ...
  async function handleAuthCallback(req, res) {
    ...
    try {
      ...
      try {
        await flushUserStoreIfSupported();
      } catch (persistErr) {
        routeLog('❌', C.red, '登录阶段持久化用户失败: ' + String((persistErr && persistErr.message) || persistErr || ''));
        return sendErrorPage(res, 500, '登录失败', '用户数据写入失败，请稍后重试');
      }
  ...
  async function handleProfile(req, res) {
    ...
    if (!user && discordUserId && typeof userStore.createOrUpdateUser === 'function') {
      routeLog('⚠️', C.yellow, 'profile 查询时用户记录缺失，自动恢复: discord=' + discordUserId);
      try {
        user = await maybeAwait(userStore.createOrUpdateUser({
          discord_user_id: discordUserId,
          username: String((current.session && current.session.username) || ''),
          global_name: String((current.session && current.session.global_name) || ''),
          avatar: String((current.session && current.session.avatar) || ''),
        }));
      } catch (recoveryErr) {
        routeLog('❌', C.red, 'profile 用户恢复失败: ' + recoveryErr.message);
      }
    }
    user = toObject(user);
  ...
  async function handleRotateApiKey(req, res) {
    ...
    if (typeof userStore.createOrUpdateUser === 'function') {
      try {
        await maybeAwait(userStore.createOrUpdateUser({
          discord_user_id: discordUserId,
          username: String((current.session && current.session.username) || ''),
          global_name: String((current.session && current.session.global_name) || ''),
          avatar: String((current.session && current.session.avatar) || ''),
        ...
        routeLog('⚠️', C.yellow, '轮换 API key 前确保用户存在失败: ' + ensureErr.message);
      }
    }
    ...
    try {
      ...
      try {
        ...
        routeLog('⚠️', C.yellow, '轮换 API key 后持久化失败: ' + String((persistErr && persistErr.message) || persistErr || ''));
      }
      ...
      var isUserMissing = /用户不存在|user.*not.*found|not.*exist/i.test(errMessage);
      if (isUserMissing && typeof userStore.createOrUpdateUser === 'function') {
        routeLog('⚠️', C.yellow, '轮换 API key 时用户记录缺失，尝试恢复: discord=' + discordUserId);
        try {
          await maybeAwait(userStore.createOrUpdateUser({
            discord_user_id: discordUserId,
            username: String((current.session && current.session.username) || ''),
          ...
          var retryResult = await maybeAwait(userStore.rotateApiKey(discordUserId));
          var retryApiKey = resolveApiKeyValue(retryResult);
          if (!retryApiKey) {
            return json(res, 500, { success: false, error: 'rotate_api_key_failed_after_recovery' });
          }
          try {
            await flushUserStoreIfSupported();
          } catch (persistRetryErr) {
            routeLog('⚠️', C.yellow, '恢复后轮换 API key 持久化失败: ' + String((persistRetryErr && persistRetryErr.message) || persistRetryErr || ''));
          }
          routeLog('🔁', C.cyan, '用户恢复后轮换 API key 成功: discord=' + discordUserId);
          return json(res, 200, {
            success: true,
            data: {
              api_key: retryApiKey,
              masked: maskApiKey(retryApiKey),
              one_time_visible: true,
            },
          });
        } catch (recoveryErr) {
          routeLog('❌', C.red, '轮换 API key 恢复流程也失败: ' + String((recoveryErr && recoveryErr.message) || recoveryErr || ''));
          return json(res, 500, { success: false, error: 'rotate_api_key_recovery_failed' });
        }
      }
      routeLog('❌', C.red, '轮换 API key 失败: ' + errMessage);
      return json(res, 500, { success: false, error: 'rotate_api_key_exception', detail: errMessage });
    }
  }

  async function handleUsageCompat(req, res) {
    var current = await requireSession(req, res);
    if (!current) return;
    ...
    return json(res, 200, { success: true, data: usage });
  }

  async function handleHistoryCompat(req, res) {
    var current = await requireSession(req, res);
    if (!current) return;
  ...
  async function handleRequest(req, res, pathname, method) {
    ...
    if (m === 'GET' && path === '/auth/api/profile') {
      await handleProfile(req, res);
      return true;
    }
    if (m === 'GET' && path === '/auth/api/usage') {
      await handleUsageCompat(req, res);
      return true;
    }
    if (m === 'GET' && path === '/auth/api/history') {
      await handleHistoryCompat(req, res);
      return true;
    }
