...
import * as gemini from '../lib/converter/gemini.mjs';
...
// 路由日志辅助
var TAG = C.green + '[gemini]' + C.reset;
function rlog(msg) {
...
export function createGeminiRoutes(ctx) {
  return async function (req, res) {
    var path = req.url.split('?')[0];

    // 匹配 /v1beta/models/{model}:streamGenerateContent 和 :generateContent
    if (req.method === 'POST' && path.startsWith('/v1beta/')) {
      var isStream = path.indexOf(':streamGenerateContent') >= 0;
      return handleGemini(req, res, ctx, path, isStream);
    }
...
async function handleGemini(req, res, ctx, path, isStream) {
  var body;
  try {
  ...
  // 从路径提取模型名
  var pathModel = '';
  var match = path.match(/\/v1beta\/models\/([^:]+)/);
  if (match) {
    pathModel = match[1];
  }
  ...
  var auth = authenticateApiKey(req, ctx.config, {
    sourceOrder: ['query.key', 'authorization', 'x-api-key'],
  });
  if (!auth.ok) {
    res.writeHead(auth.status || 401, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: { message: ctx.t('request.auth_invalid') } }));
    return;
  }
  req._apiKeyIdentity = auth.identity;

  body._stream = isStream;
  body.model = pathModel || body.model;

  var universal = gemini.parseRequest(body);
  ...
  // 请求进入日志
  rlog('POST ' + path + ' | model=' + C.bold + universal.model + C.reset + ' | stream=' + isStream);
  ...
  var retryPolicy = resolveRetryPolicy(ctx.config, isStream);
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      if (!upstreamResp.ok) {
        ...
        res.end(JSON.stringify({ error: { message: 'Upstream error: ' + upstreamResp.status } }));
        return;
      }
      ...
      if (isStream) {
        var sseCtx = { model: prefixedModel };
          ...
          await parseSSEStream(upstreamResp.body, function (eventType, data) {
            ...
            for (var ei = 0; ei < events.length; ei++) {
              ...
              if (parsedEvent.type === 'error') {
                streamFailed = true;
                streamErrorType = (parsedEvent.error && (parsedEvent.error.code || parsedEvent.error.type)) || 'upstream_stream_error';
              ...
              var chunk = gemini.formatSSEChunk(parsedEvent, sseCtx);
        ...
        if (streamFailed) {
          ...
          if (!responseStarted && !res.headersSent) {
            ...
            res.end(JSON.stringify({ error: { message: streamErrorMessage || 'Upstream stream error' } }));
        ...
        if (chunksSent === 0 && isUsageEmpty(totalUsage)) {
          ...
          if (!res.writableEnded) res.end(JSON.stringify({ error: { message: 'Upstream returned empty stream' } }));
          return;
        }
      ...
      if (nonStreamError) {
        ...
        res.end(JSON.stringify({ error: { message: nonStreamError.message || 'Upstream error' } }));
        return;
      }

      var jsonResp = gemini.formatNonStreamResponse(resp);
  ...
  if (!res.writableEnded) res.end(JSON.stringify({ error: { message: ctx.t('scheduler.no_available') } }));
}
