...
export class RuleEngine {
  constructor(opts) {
    ...
    this._getPersistedIdentityStats = typeof options.getPersistedIdentityStats === 'function'
      ? options.getPersistedIdentityStats
      : null;
  ...
  _mergePublicIdentityStats(runtimeStats, persistedStats) {
    var runtime = this._toPublicIdentityStats(runtimeStats);
    var persisted = this._toPublicIdentityStats(persistedStats);
    var firstSeen = 0;
    if (runtime.first_seen > 0 && persisted.first_seen > 0) {
      firstSeen = Math.min(runtime.first_seen, persisted.first_seen);
    } else if (runtime.first_seen > 0) {
      firstSeen = runtime.first_seen;
    } else if (persisted.first_seen > 0) {
      firstSeen = persisted.first_seen;
    }
    return {
      requests: Math.max(runtime.requests, persisted.requests),
      input_tokens: Math.max(runtime.input_tokens, persisted.input_tokens),
      output_tokens: Math.max(runtime.output_tokens, persisted.output_tokens),
      cached_tokens: Math.max(runtime.cached_tokens, persisted.cached_tokens),
      first_seen: firstSeen,
      last_seen: Math.max(runtime.last_seen, persisted.last_seen),
    };
  }

  _loadPersistedIdentityStats() {
    var out = new Map();
    if (typeof this._getPersistedIdentityStats !== 'function') {
      return out;
    }

    var rawStats = null;
    try {
      rawStats = this._getPersistedIdentityStats();
    } catch (_) {
      return out;
    }
    ...
    var self = this;
    function applyStats(identityHint, row) {
      ...
      var current = out.get(identity);
      if (!current) {
        current = self._createIdentityStats(0);
        current.first_seen = 0;
        current.last_seen = 0;
        current._feature_tokens_24h = 0;
      }

      current.requests += Math.max(0, Math.floor(toFiniteNumber(source.requests, 0)));
      current.input_tokens += Math.max(0, Math.floor(toFiniteNumber(
        source.input_tokens !== undefined ? source.input_tokens : source.input,
        0
      )));
      current.output_tokens += Math.max(0, Math.floor(toFiniteNumber(
        source.output_tokens !== undefined ? source.output_tokens : source.output,
        0
      )));
      current.cached_tokens += Math.max(0, Math.floor(toFiniteNumber(
        source.cached_tokens !== undefined ? source.cached_tokens : source.cached,
        0
      )));

      var firstSeen = Math.max(0, Math.floor(toFiniteNumber(
        source.first_seen !== undefined ? source.first_seen : source.firstSeen,
        0
      )));
      var lastSeen = Math.max(0, Math.floor(toFiniteNumber(
        source.last_seen !== undefined ? source.last_seen : source.lastSeen,
        0
      )));
      if (firstSeen > 0 && (current.first_seen <= 0 || firstSeen < current.first_seen)) {
        current.first_seen = firstSeen;
      }
      if (lastSeen > current.last_seen) {
        current.last_seen = lastSeen;
      }
      out.set(identity, current);
    }

    if (rawStats instanceof Map) {
      for (var entry of rawStats.entries()) {
        applyStats(entry[0], entry[1]);
      }
      return out;
    }
  ...
  listUsers(options) {
    ...
    var persistedStatsMap = this._loadPersistedIdentityStats();
    ...
    for (var identityInPersisted of persistedStatsMap.keys()) {
      ensureProfile(identityInPersisted);
    }
    ...
    for (var profileRow of profiles.values()) {
      ...
      var runtimeStats = this._toPublicIdentityStats(this._allIdentities.get(resolvedIdentity));
      var persistedStats = this._toPublicIdentityStats(persistedStatsMap.get(resolvedIdentity));
      var stats = this._mergePublicIdentityStats(runtimeStats, persistedStats);
  ...
  getOverview() {
    ...
    var persistedStatsMap = this._loadPersistedIdentityStats();
    for (var identityInPersisted of persistedStatsMap.keys()) {
      identitySet.add(identityInPersisted);
    }
    if (typeof this._getRegisteredUsers === 'function') {
      try {
  ...
  getUserStats(identityOrId) {
    var identity = this._resolveIdentityFromInput(identityOrId);
    if (!identity) return null;
    var runtimeStats = this._toPublicIdentityStats(this._allIdentities.get(identity));
    var persistedStatsMap = this._loadPersistedIdentityStats();
    var persistedStats = this._toPublicIdentityStats(persistedStatsMap.get(identity));
    var merged = this._mergePublicIdentityStats(runtimeStats, persistedStats);
    if (
      merged.requests <= 0
      && merged.input_tokens <= 0
      && merged.output_tokens <= 0
      && merged.cached_tokens <= 0
      && merged.first_seen <= 0
      && merged.last_seen <= 0
    ) {
      return null;
