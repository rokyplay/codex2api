...
export class RuleEngine {
  ...
  listUsers(options) {
    ...
    for (var identity of identitySet.values()) {
      var state = this._states.get(identity);
      if (!state) {
        state = this._buildBaseState(identity, getDefaultFeatures(identity), Date.now());
      }
      var stats = this._toPublicIdentityStats(this._allIdentities.get(identity));
      var item = {
        id: identity,
        caller_identity: identity,
  ...
  getOverview() {
    ...
    var todayEvents = this._riskLogger && typeof this._riskLogger.getTodayCount === 'function'
      ...
      : 0;
    var totalUsers = this._allIdentities.size;
